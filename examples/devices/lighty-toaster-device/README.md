...
**Run device**
* extract binary distribution `lighty-toaster-device-24.1.0-SNAPSHOT-bin.zip`
from target directory
* run jar file from zip with default parameter
```
java -jar lighty-toaster-device-24.1.0-SNAPSHOT.jar
```
To run device on specific port, add port number as an argument
* run device on specific port `12345` (any available port)
```
java -jar lighty-toaster-device-24.1.0-SNAPSHOT.jar 12345
```
...
`java -jar lighty-toaster-device-24.1.0-SNAPSHOT.jar -i path/to/persistence/directory -o path/to/persistence/directory`
